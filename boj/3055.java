...
public class p3055 {
    private static class EscapeSolver {
        private final char startCharacter = 'S';
        private final char endCharacter = 'D';
        private final char wallCharacter = '#';
        private final char emptyCharacter = '.';
        private final char fireCharacter = '*';
        ...
        public EscapeSolver() throws IOException {
            ...
            for (int i = 0; i < n+2; i++) {
                map[i][0] = wallCharacter;
                map[i][m+1] = wallCharacter;
            }
            for (int i = 0; i < m+2; i++) {
                map[0][i] = wallCharacter;
                map[n+1][i] = wallCharacter;
            }
            for (int i = 1; i < n+1; i++) {
